              ...
              >
                ...
                <template v-slot:selected>
                  <div v-if="selectedStation">
                    {{ stationOptions.find(opt => opt.value === selectedStation)?.description || `工位 ${selectedStation}` }}
                  </div>
                </template>
              </q-select>
            </div>
...
// 工位选择
const selectedStation = ref<number | null>(1)
const stationOptions = ref([
  { value: 1, name: '工位1', description: '底边刃口+下表面检测' },
...
// 确保选中的是数值而不是对象
const selectStation = (value: number | object) => {
  // 如果接收到的是对象，尝试从中提取value属性
  const stationValue = typeof value === 'object' && value !== null ? (value as any).value : value;
  // 确保最终存储的是数值
  selectedStation.value = typeof stationValue === 'number' ? stationValue : null;
}
...
const getStreamUrl = (camera: Device | null): string => {
  ...
  // 构建API流地址
  const streamUrl = cameraService.getCameraStreamUrl(cameraId)
