...
// 初始化编辑器（暂时用简单文本区域替代monaco-editor）
const initEditor = async () => {
  ...
  try {
    ...
    disposeEditor = () => {
      textarea.removeEventListener('input', handleEditorChange)
      container.innerHTML = ''
    }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const currentLine = textarea.value.substring(0, cursorPos).split('\n').length
      
      if (currentLine === 1 && lines[0].startsWith('def process(')) {
        // 阻止删除或修改第一行
        if (['Backspace', 'Delete', 'Insert'].includes(e.key) || e.key.match(/^[a-zA-Z0-9]$/)) {
          e.preventDefault()
        }
      }
    }
    ...
    // 更新disposeEditor函数
    const originalDispose = disposeEditor
    disposeEditor = () => {
      originalDispose?.()
