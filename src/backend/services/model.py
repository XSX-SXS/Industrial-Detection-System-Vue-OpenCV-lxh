...
from ..common.globals import TRAINING_LOGS, TRAINING_PROCESSES, TRAINING_YOLO8_MODELS
import sys
...
class ModelService:
    ...
    def delete_model(self, model_id: int) -> None:
        ...
        try:
            ...
            if os.path.exists(model_dir) and os.path.isdir(model_dir):
                ...
                logger.info(f"已删除模型目录: {model_dir}")
            
            # 2. 然后检查并删除数据库中记录的模型文件（以防目录删除失败时仍能删除文件）
            if model.file_path and os.path.exists(model.file_path):
                os.remove(model.file_path)
                logger.info(f"已删除模型文件: {model.file_path}")
            
            # 3. 从缓存中删除（使用安全的方式访问_loaded_models属性）
            if hasattr(self, '_loaded_models') and model_id in self._loaded_models:
                del self._loaded_models[model_id]
                logger.info(f"已从缓存中删除模型: {model_id}")
            ...
            self.db.rollback()
            logger.error(f"删除模型时出错: {str(e)}", exc_info=True)
    ...
    def stop_training(self, model_id: int) -> Model:
        """停止模型训练"""
        model = self.get_model(model_id)
        if not model:
            raise HTTPException(status_code=404, detail="Model not found")
        ...
        # 检查模型状态，如果不是训练中则返回错误
        if model.status != ModelStatus.TRAINING:
            raise HTTPException(status_code=400, detail="Model is not training")
            
        try:
            # 停止YOLOv8训练（使用API方式）
            if model_id in TRAINING_YOLO8_MODELS:
                # 对于YOLOv8，尝试停止训练
                logger.info(f"尝试停止模型 {model_id} 的YOLOv8训练")
                try:
                    # Ultralytics YOLO模型没有直接的停止方法，需要通过进程终止
                    # 这里我们清理全局变量，让训练自然停止
                    del TRAINING_YOLO8_MODELS[model_id]
                except Exception as e:
                    logger.error(f"停止YOLOv8训练失败: {str(e)}")
            
            # 停止YOLOv5训练（使用子进程方式）
            if model_id in TRAINING_PROCESSES:
                logger.info(f"尝试停止模型 {model_id} 的训练进程")
                process = TRAINING_PROCESSES[model_id]
                
                try:
                    # 对于Windows系统，使用terminate()方法
                    process.terminate()
            ...
            if model_id in TRAINING_LOGS:
                ...
                TRAINING_LOGS[model_id].append(f"[{timestamp}] [系统] 训练已被用户停止")
    ...
    def _run_yolo_training(self, model_id: int, dataset_dir: str, parameters: dict, model_type: str) -> bool:
        ...
        try:
            ...
            if model_type == "YOLOv8":
                # 使用Ultralytics Python API进行训练，更可靠
                try:
                    # 直接使用Python API训练
                    from ultralytics import YOLO
                    ...
                    # 保存训练对象到全局变量，以便可以停止训练
                    TRAINING_YOLO8_MODELS[model_id] = model
                    ...
                    try:
                        ...
                        # 记录训练结果
                        TRAINING_LOGS[model_id].append(f"[{timestamp}] [系统] 训练完成！")
                        ...
                        if os.path.exists(best_model_path):
                            ...
                            success_msg = f"{model_type}训练完成，模型保存在: {best_model_path}"
                        ...
                        logger.error(f"使用Ultralytics API训练失败: {str(e)}", exc_info=True)
                        TRAINING_LOGS[model_id].append(f"[{timestamp}] [错误] 训练失败: {str(e)}")
                        return False
                finally:
                    # 清理全局变量
                    if model_id in TRAINING_YOLO8_MODELS:
    ...
    def __init__(self, db: Session):
        """初始化模型服务"""
        self.db = db
        self._label_to_class_map = {}  # 缓存标签到类别ID的映射（实例变量）
        self._class_names = []         # 保存所有类别名称的列表（实例变量）
