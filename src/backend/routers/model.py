...
@router.post("/{model_id}/stop-training", response_model=ModelResponse)
def stop_training(model_id: int, db: Session = Depends(get_db)):
    service = ModelService(db)
    return service.stop_training(model_id)
